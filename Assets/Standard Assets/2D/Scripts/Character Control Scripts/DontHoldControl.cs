...
namespace UnityStandardAssets._2D
{
    public class DontHoldControl : MonoBehaviour
    {
		...
		public float maxTimeKeyHeld = 2f;
        ...
        public void Move(float move, bool jump) {
            ...
            if (m_Grounded || m_AirControl) {
				...
				// Move the character
				m_Rigidbody2D.velocity = new Vector2(move * m_MaxSpeed * Mathf.Max(0, 1 - (timeKeyHeld / maxTimeKeyHeld)) , m_Rigidbody2D.velocity.y);
        ...
        private void Update()
        {
			...
			if (Input.anyKey) {
				timeKeyHeld = Mathf.Min(timeKeyHeld + Time.deltaTime, maxTimeKeyHeld);
			} else {
				timeKeyHeld = Mathf.Max(timeKeyHeld - (Time.deltaTime * coolDownSpeed), 0);
			}
